...
module.exports = {
    ...
    callback: async (message, args) => {
        ...
        if (!searchString) return sendError("Please tell me what you want to play. (Youtube links might not work)", message.channel);

        var serverQueue = message.client.queue.get(message.guild.id);

        var searched = await yts.search(searchString)
        if (searched.videos.length === 0) return sendError("Looks like i was unable to find the song on YouTube. Please make sure you entered a valid query.", message.channel)
